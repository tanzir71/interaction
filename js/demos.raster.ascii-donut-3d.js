...
.d-ascii-donut:before{content:'';position:absolute;inset:0;z-index:10;pointer-events:none;background:radial-gradient(circle at center,rgba(167,139,250,.045),transparent 46%)}
.d-ascii-donut:focus-visible{box-shadow:inset 0 0 0 2px rgba(167,139,250,.72)}
...
.d-ascii-donut-live{display:flex;align-items:center;gap:6px;color:#9b9ba3}.d-ascii-donut-live i{width:5px;height:5px;border-radius:50%;background:#a78bfa;box-shadow:0 0 8px rgba(167,139,250,.4)}
...
Find occupied silhouette-edge cells in the z-buffer, propagate their inward four-neighbor grid distance, and mix #a78bfa at exactly 30 percent into every occupied cell whose distance from that edge is zero through six. Base ink interpolates between #9b9ba3 and #ececef from luminance. Keep the exact glyph choice driven only by the transformed normal luminance. Batch cells by their computed color before calling fillText, cap the font at 11px, and shrink it responsively so the widest measured glyph never exceeds one cell. Add compact 10px labels, 12px angle, grid, and ramp data, cell and drag-vector telemetry around the canvas with one visible keyboard focus target and a polite status. Use the shared 10px panel radius and do not add FUI scanlines or hard-tech corner brackets.
