...
.d-paper-stack {
  ...
  padding: 21px clamp(14px,4vw,38px) 18px; color: #28251f; background: #837b6e;
  font-family: Inter,system-ui,sans-serif;
}
.d-paper-stack::before { content: ''; position: absolute; inset: 0; pointer-events: none; opacity: .55; background: repeating-linear-gradient(6deg,rgba(255,255,255,.025) 0 1px,rgba(35,29,21,.025) 1px 2px,transparent 2px 7px),radial-gradient(circle at 25% 0,#aaa191,transparent 48%); }
.d-paper-stack-head { position: relative; z-index: 5; display: flex; justify-content: space-between; gap: 14px; color: #d5cdbf; font: 700 9px/1.2 ui-monospace,SFMono-Regular,Consolas,monospace; letter-spacing: .14em; }.d-paper-stack-head span:last-child { display: inline-flex; align-items: center; gap: 7px; }.d-paper-stack-head i { width: 6px; height: 6px; border-radius: 50%; background: #e16b4d; box-shadow: 0 0 7px rgba(225,107,77,.45); }
.d-paper-stack-desk { position: relative; z-index: 2; width: min(700px,100%); height: 385px; margin: 17px auto 0; overflow: hidden; border: 1px solid rgba(55,47,38,.38); border-radius: 4px; background: repeating-linear-gradient(90deg,rgba(255,255,255,.025) 0 1px,transparent 1px 5px),linear-gradient(145deg,#766e62,#625a50); box-shadow: inset 0 2px 8px rgba(40,33,26,.42),0 14px 28px rgba(47,39,31,.2); }
.d-paper-stack-counter { position: absolute; left: 16px; top: 15px; z-index: 8; display: grid; gap: 4px; color: #a9a092; font: 650 7px/1 ui-monospace,SFMono-Regular,Consolas,monospace; letter-spacing: .13em; }.d-paper-stack-counter strong { color: #ded5c6; font-size: 14px; letter-spacing: .08em; }
...
.d-paper-card {
  ...
  border: 1px solid rgba(92,81,66,.22); border-radius: 1px; color: #2c2923; background: #eee8da; outline: none;
  box-shadow: 0 2px 3px rgba(43,36,28,.18),0 10px 20px rgba(43,36,28,.16); user-select: none; touch-action: none; cursor: grab;
  transform-origin: 50% 72%; will-change: transform;
}
.d-paper-card::before { content: ''; position: absolute; inset: 0; pointer-events: none; opacity: .62; background: repeating-linear-gradient(2deg,rgba(74,62,46,.025) 0 1px,transparent 1px 5px),radial-gradient(circle at 20% 0,rgba(255,255,255,.72),transparent 42%); }
.d-paper-card::after { content: ''; position: absolute; right: 0; bottom: 0; width: calc(12px + var(--peel) * 40px); height: calc(12px + var(--peel) * 40px); pointer-events: none; background: linear-gradient(135deg,rgba(112,96,75,.28) 0 48%,#faf7ef 50% 68%,rgba(70,58,43,.22) 70%); clip-path: polygon(100% 0,0 100%,100% 100%); filter: drop-shadow(-2px -2px 2px rgba(64,51,38,.13)); }
.d-paper-card > * { position: relative; z-index: 2; }.d-paper-card header,.d-paper-card footer { display: flex; align-items: center; justify-content: space-between; gap: 12px; font: 700 8px/1 ui-monospace,SFMono-Regular,Consolas,monospace; letter-spacing: .12em; }.d-paper-card header { padding-bottom: 12px; border-bottom: 1px solid rgba(74,65,53,.2); color: #71695e; }.d-paper-card header b { color: #bb624d; font-weight: 700; }.d-paper-card h3 { margin: 22px 0 8px; font: 700 clamp(24px,4.8vw,38px)/.94 Georgia,'Times New Roman',serif; letter-spacing: -.035em; }.d-paper-card p { margin: 0; color: #81786c; font: 500 10px/1.4 ui-monospace,SFMono-Regular,Consolas,monospace; }.d-paper-card footer { position: absolute; left: 24px; right: 24px; bottom: 18px; justify-content: flex-start; color: #8e8578; }.d-paper-card footer i { width: 20px; height: 2px; background: #c36b55; }
.d-paper-card.is-top { z-index: 4; transform: translate3d(0,0,0) rotate(0deg); }.d-paper-card.is-mid { z-index: 3; transform: translate3d(10px,10px,0) rotate(2.1deg); background: #e4ddce; }.d-paper-card.is-back { z-index: 2; transform: translate3d(-8px,19px,0) rotate(-2.6deg); background: #d8d0c1; }
.d-paper-card.is-top:focus-visible { box-shadow: 0 0 0 3px #6d655a,0 0 0 5px #eee8da,0 14px 25px rgba(43,36,28,.24); }.d-paper-card.is-dragging { cursor: grabbing; z-index: 7; box-shadow: 0 5px 6px rgba(43,36,28,.15),0 25px 42px rgba(43,36,28,.28); }.d-paper-card.is-tossing { z-index: 9; pointer-events: none; }
.d-paper-card-b { color: #26302d; background: #e6e8df; }.d-paper-card-b header b,.d-paper-card-b footer i { color: #427d70; background: #427d70; }.d-paper-card-c { color: #2d2931; background: #e7e0e7; }.d-paper-card-c header b,.d-paper-card-c footer i { color: #7d5f84; background: #7d5f84; }
.d-paper-stack-threshold { position: absolute; right: 16px; bottom: 14px; z-index: 8; display: flex; align-items: center; gap: 7px; color: #ada496; font: 650 7px/1 ui-monospace,SFMono-Regular,Consolas,monospace; letter-spacing: .09em; }.d-paper-stack-threshold i { width: 18px; border-top: 1px dashed #c2b8a8; }
.d-paper-stack-foot { position: relative; z-index: 5; width: min(700px,100%); margin: 13px auto 0; display: flex; align-items: center; justify-content: space-between; gap: 13px; }.d-paper-stack-status { margin: 0; color: #ded5c6; font: 650 9px/1.3 ui-monospace,SFMono-Regular,Consolas,monospace; letter-spacing: .055em; }.d-paper-stack-foot > div { display: flex; gap: 6px; }.d-paper-stack button { min-height: 31px; padding: 7px 10px; border: 1px solid #a69d8f; border-radius: 2px; color: #e5dccd; background: rgba(83,75,65,.65); font: 700 8px/1 ui-monospace,SFMono-Regular,Consolas,monospace; letter-spacing: .07em; text-transform: uppercase; cursor: pointer; }.d-paper-stack button:hover,.d-paper-stack button:focus-visible { color: #fff; border-color: #e0d6c7; outline: none; }
