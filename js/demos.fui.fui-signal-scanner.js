...
INTRX.register({
  ...
  <section class="d-fui-scan-panel" tabindex="0" role="slider" aria-label="Selected radio band" aria-valuemin="88" aria-valuemax="168.8" aria-valuenow="128.4" aria-valuetext="128.4 megahertz">
  ...
  <footer class="d-fui-scan-footer"><span>64 BANDS</span><span>Δ 1.26MHz</span><span>SWEEP 3.2S</span><span class="d-fui-scan-status">CLEAR</span></footer>
...
.d-fui-scan-topbar,.d-fui-scan-footer{position:relative;z-index:10;display:flex;align-items:center;justify-content:space-between;color:#5c5c66;font-size:9px;line-height:1;letter-spacing:.08em}.d-fui-scan-live{display:flex;align-items:center;gap:6px;color:#9b9ba3}.d-fui-scan-live-dot{width:5px;height:5px;border-radius:50%;background:#a78bfa;box-shadow:0 0 9px rgba(167,139,250,.35);animation:d-fui-scan-live-pulse 1.6s ease-in-out infinite}
.d-fui-scan-panel{position:relative;z-index:2;min-height:0;overflow:hidden;border:1px solid #232327;border-radius:4px;background:#101012;outline:none;cursor:crosshair}.d-fui-scan-panel:focus-visible{box-shadow:inset 0 0 0 2px rgba(167,139,250,.5)}.d-fui-scan-panel-head{position:absolute;z-index:6;left:10px;right:10px;top:8px;display:flex;justify-content:space-between;color:#5c5c66;font-size:8px;line-height:1;letter-spacing:.08em}.d-fui-scan-mode{color:#9b9ba3}
.d-fui-scan-plot{position:absolute;left:50%;bottom:27px;width:191px;height:194px;transform:translateX(-50%)}.d-fui-scan-bars{position:absolute;left:0;right:0;bottom:0;height:174px;display:flex;align-items:flex-end;column-gap:1px}.d-fui-scan-bar{position:relative;display:block;flex:0 0 2px;width:2px;height:var(--d-fui-scan-height);background:#5c5c66;transform:scaleY(1);transform-origin:50% 100%;transition:background-color 160ms linear;will-change:transform,background-color}.d-fui-scan-bar.d-fui-scan-is-swept{background:#ececef}.d-fui-scan-bar.d-fui-scan-is-selected{background:#67e8f9}
.d-fui-scan-tip{position:absolute;left:0;top:0;width:2px;height:2px;background:#a78bfa}.d-fui-scan-bar.d-fui-scan-is-selected .d-fui-scan-tip{background:#67e8f9}.d-fui-scan-blip{position:absolute;left:-1px;top:-7px;width:4px;height:4px;border-radius:50%;background:#f87171;box-shadow:0 0 9px rgba(248,113,113,.45);opacity:0;transform:scale(.6);will-change:opacity,transform}
.d-fui-scan-sweep{position:absolute;z-index:4;left:0;top:20px;bottom:0;width:1px;background:#a78bfa;box-shadow:0 0 9px rgba(167,139,250,.55);will-change:transform}.d-fui-scan-band{position:absolute;z-index:5;left:0;top:0;width:25px;height:18px;pointer-events:none;will-change:transform}.d-fui-scan-bracket{position:absolute;left:0;bottom:0;width:25px;height:12px;box-sizing:border-box;border-left:1px solid #a78bfa;border-right:1px solid #a78bfa;border-top:1px solid #a78bfa}.d-fui-scan-frequency{position:absolute;left:50%;bottom:16px;padding:3px 4px;border-radius:2px;background:rgba(10,10,11,.86);color:#67e8f9;font:8px/1 'JetBrains Mono',monospace;letter-spacing:.03em;transform:translateX(-50%);white-space:nowrap;font-variant-numeric:tabular-nums}
.d-fui-scan-scale{position:absolute;left:calc(50% - 96px);bottom:10px;width:192px;display:flex;justify-content:space-between;color:#5c5c66;font-size:7px;line-height:1}.d-fui-scan-footer{font-size:8px;border-top:1px solid #232327}.d-fui-scan-status{color:#4ade80}.d-fui-scan.d-fui-scan-has-contact .d-fui-scan-status{color:#f87171}
.d-fui-scan-corner{position:absolute;z-index:7;width:8px;height:8px;pointer-events:none}.d-fui-scan-corner-tl{left:3px;top:3px;border-left:2px solid #2e2e34;border-top:2px solid #2e2e34}.d-fui-scan-corner-tr{right:3px;top:3px;border-right:2px solid #2e2e34;border-top:2px solid #2e2e34}.d-fui-scan-corner-bl{left:3px;bottom:3px;border-left:2px solid #2e2e34;border-bottom:2px solid #2e2e34}.d-fui-scan-corner-br{right:3px;bottom:3px;border-right:2px solid #2e2e34;border-bottom:2px solid #2e2e34}.d-fui-scan.d-fui-scan-is-paused .d-fui-scan-corner{border-color:#a78bfa}
.d-fui-scan-announcement{position:absolute;width:1px;height:1px;overflow:hidden;clip:rect(0 0 0 0)}.d-fui-scan-scanline{position:absolute;z-index:8;left:13px;right:13px;top:31px;height:1px;background:#a78bfa;opacity:.07;pointer-events:none;animation:d-fui-scan-stage-line 5.2s linear infinite}
...
@media(prefers-reduced-motion:reduce){.d-fui-scan *{animation:none!important;transition:none!important}.d-fui-scan-live-dot{opacity:1}.d-fui-scan-scanline{top:160px}.d-fui-scan-bar.d-fui-scan-is-contact .d-fui-scan-blip{opacity:1;transform:scale(1)}}
`,
...
function updateContact(){
  ...
  blip.style.transform='scale('+(1+.35*Math.sin(progress*Math.PI)-.4*progress).toFixed(3)+')';
  root.dataset.contactProgress=progress.toFixed(3);
  if(simulationTime>=contactEnd){bars[index].classList.remove('d-fui-scan-is-contact');blip.style.opacity='0';blip.style.transform='scale(.6)';contactIndex=-1;announcement.textContent='Spectrum clear';expose('contact clear')}
}
...
panel.addEventListener('keydown',function(event){
  if(event.key==='ArrowLeft'||event.key==='ArrowRight'){event.preventDefault();selectedStart=Math.max(0,Math.min(barCount-6,selectedStart+(event.key==='ArrowLeft'?-1:1)));updateBand('keyboard')}
...
if(reduced){sweepElapsed=sweepDuration*.28;sweepX=plotWidth*.28;sweepNode.style.transform='translate3d('+sweepX.toFixed(2)+'px,0,0)';showContact(47,false);updateSweepClasses();paused=false;expose('reduced')}else{planContact();start()}`,
  prompt:`Build a self-contained 320px-tall fictional radio spectrum scanner using JetBrains Mono and only #0a0a0b, #101012, #232327, #2e2e34, #ececef, #9b9ba3, #5c5c66, accent #a78bfa, info #67e8f9, success #4ade80, and error #f87171. Frame the scanner as a 4px-radius FUI panel with four 8px corner ticks, scanlines, and an always-visible accent activity dot.

Render exactly 64 DOM spectrum bars, each exactly 2px wide with a 1px gap. Give every bar a seeded noise height, a #5c5c66 body, and an exact 2px #a78bfa tip. Move one exact 1px accent sweep line left to right on a linear 3.2-second cycle. Bars whose centers enter the sweep's 20px influence turn #ececef and run a one-shot 200ms sine pulse from scaleY(1) through an exact scaleY(1.15) peak and back to scaleY(1).

Map pointer x to the nearest band and select exactly six adjacent bars in #67e8f9. Position a bracket above those six bars and show a one-decimal MHz value; center the initial group at 128.4MHz. When the sweep crosses the currently selected group, freeze the sweep's elapsed clock at that exact position for 800ms, brighten the panel corners to accent, then continue without a timing jump. Support Left and Right arrows plus Home and End on the focusable slider-like panel.
